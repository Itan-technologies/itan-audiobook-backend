...
class Api::V1::ReaderController < ApplicationController
  ...
  def authenticate_reader_token
    ...
    begin
      decoded = JWT.decode(token, ENV.fetch('DEVISE_JWT_SECRET_KEY', nil), true, { algorithm: 'HS256' })
  ...
  def detect_suspicious_activity
    ...
    return unless unique_ips > 3 # Allow max 3 IPs (home + mobile + work)
  ...
  def valid_page?(page_number)
    page_number.positive? && page_number <= extract_total_pages
  end

  # NEW: Get total pages efficiently
  def extract_total_pages
    @extract_total_pages ||= if @purchase.content_type == 'ebook' && @purchase.book.ebook_file.attached?
