...
class Api::V1::Admin::AuthorsController < ApplicationController
  ...
  def render_authors_json(authors, message = nil, status_code = 200)
    ...
    response[:data] = if authors.is_a?(Author)
                        AuthorSerializer.new(authors).serializable_hash[:data][:attributes]
                      else
                        AuthorSerializer.new(authors).serializable_hash[:data].map { |author| author[:attributes] }
                      end
